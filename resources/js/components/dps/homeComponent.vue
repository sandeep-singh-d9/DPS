                        ...
                        <div v-if="dragInputs.length > 0">
                            <div v-for="(input, index) in dragInputs" :key="index" >
                                <div  draggable="true" @dragstart="dragStart(index, $event)" @dragover.prevent @dragenter="dragEnter" @dragleave="dragLeave" @dragend="dragEnd" @drop="dragFinish(index, $event)">
                                    <div v-html="input.col" :id="input.id" @click="addInputs">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div v-if="inputs.length > 0">
                            <div v-for="(input, index) in inputs" :key="index" draggable="true" @dragstart="dragStart(index, $event)" @dragover.prevent @dragenter="dragEnter" @dragleave="dragLeave" @dragend="dragEnd" @drop="dragFinish(index, $event)">
                                
                                <div v-if="input.textbox" @click="onfocusFun('textbox', index)">
                                    
                                    <div v-html="input.textbox.name" contenteditable="true" :id="input.textbox.id" :class="input.textbox.class"> Hodor!</div>
                                    
                                </div>
                                <div v-if="input.textarea">
                                    <p contenteditable="true">
                                        <textarea name="" @click="onfocusFun('textarea', index)" :id="input.textarea.id" cols="30" rows="10" v-model="input.textarea.name" :class="input.textarea.class" :style="input.textarea.style"></textarea>
                                    </p>
                                </div>
                                <div v-if="input.file">
                                    <p contenteditable="true">
                                        <img src="/image/avatar_image.jpg" @click="onfocusFun('file', index), setFileId(input.file.id)" :id="input.file.id" alt="" srcset="" width="100px" :style="input.file.style">
                                    </p>
                                </div>
                                <div v-if="input.video">
                                    
                                    <p contenteditable="true">
                                        <video :id="input.video.id" playsinline="playsinline" webkit-playsinline="true"  width="320" height="240" tabindex="-1" autoplay="" :src="input.video.src" :style="input.video.style" controls></video>
                                    </p>
                                </div>

                                <div v-if="input.videoLink">
                                    <p contenteditable="true">
                                        <iframe width="420" height="315" :src="input.videoLink.src" :style="input.videoLink.style" frameborder="0" allowfullscreen></iframe>
                                    </p>
                        ...
                        <div class="card-body">
                            ...
                            <div class="tab-content py-5 px-5 px-sm-0" id="nav-tabContent">
                                <div v-if="tabSelected == 1" >
                                    ...
                                    <button @click="addHtml(2)">2 Column</button>
                                    <button @click="addHtml(3)">3 Column</button>
                                    <button @click="addRow('textbox')">Textbox</button>
                                    <button @click="addRow('textarea')">Textarea</button>
                                    <button @click="addRow('file')">File</button>
                                    <button @click="addRow('video')">Video</button>
                                    <button @click="addRow('videoLink')">Youtube Link</button>
                                ...
                                <div v-if="tabSelected == 2">
                                    <input v-if="inputType == 'file'" type="file" name=""  @change="uploads" > 
                                    Width: <input type="text" :value="width" @input="changeInput" placeholder="Width" name="width" title="width accept % and px" class="form-control">
                                    Height: <input type="text" :value="height" @input="changeInput" placeholder="Width" name="height" title="Height accept % and px" class="form-control">
                                    ...
                                    <input v-if="inputType == 'video'" type="text" v-model="liveLink" class="form-control" placeholder="Live Link" >
                                    <input v-if="inputType == 'videoLink'" type="text" v-model="youtubeId" class="form-control" placeholder="Youtube Id" >
                                    <button @click="addfont('italic', false, null)"><i>I</i></button>
                                    <button @click="addfont('bold', false, null)"><i>B</i></button>
                                    <button @click="addfont('underline', false, null)"><u>U</u></button>
    ...
    methods: {
        ...
        addRow (inputType) {
            ...
            if (inputType == 'textbox') {
                this.inputs.push({ textbox: { name: 'Valar morghulis! Valar dohaeris!', id: 'input_'+id, class: '', style: ''} })
            }
            if (inputType == 'textarea') {
                this.inputs.push({ textarea: { name: '', id: 'teaxarea_'+id, class: 'form-control', style: ''} })
            }
            if (inputType == 'file') {
                this.inputs.push({ file: { name: '', id: fileId, class: '', style: '', src: ''} })
            }
            if (inputType == 'video') {
                this.inputs.push({ video: { name: '', id: fileId+1, class: '', style: '', src: ''} })
            }
            if (inputType == 'videoLink') {
                this.inputs.push({ videoLink: { name: '', id: fileId+11, class: '', style: '', src: 'https://www.youtube.com/embed/'} })
            }
        ...
        saveData () {
            ...
            console.log(this.inputs, 'kandarp')
        ...
        applyCss () {
            ...
            if (this.inputType == 'videoLink') {
                ...
                this.inputs[this.indexs].videoLink.src = 'https://www.youtube.com/embed/'+ this.youtubeId
            }
        ...
        addHtml (type) {
            ...
            var id3 =  Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
            if (type === 1) {
                this.dragInputs.push({col: '<div class="row"><div class="col-md-12 cell" @click="addInupts" contenteditable="true" id="'+id1+'"><p>kandarp pandya</p></div></div>', id: id})
            }
            if (type === 2) {
                this.dragInputs.push({col: '<div class="row"><div class="col-md-6 cell" contenteditable="true" @click="addInupts" id="'+id1+'"><p>kandarp</p></div><div class="col-md-6 cell" contenteditable="true" @click="addInupts" id="'+id2+'"><p>kandarp</p></div></div>', id: id})
            }
            if (type === 3) {
                this.dragInputs.push({col: '<div class="row"><div class="col-md-4 cell" contenteditable="true" @click="addInupts" id="'+id1+'"><p>kandarp</p></div><div class="col-md-4 cell" contenteditable="true" @click="addInupts" id="'+id3+'"><p>kandarp</p></div><div class="col-md-4 cell" contenteditable="true" @click="addInupts" id="'+id3+'"><p>kandarp</p></div></div>', id: id})
            }
        },
        ...
        getChildPayload1 (index) {
            ...
            var data = ''
            if (index === 0) {
                data = '<div class="row"><div class="col-md-12 cell" @click="addInupts" contenteditable="true" id="'+id1+'"><p>kandarp pandya</p></div></div>'
            }
            if (index === 1) {
                data = '<div class="row"><div class="col-md-6 cell" contenteditable="true" @click="addInupts" id="'+id1+'"><p>kandarp</p></div><div class="col-md-6 cell" contenteditable="true" @click="addInupts" id="'+id2+'"><p>kandarp</p></div></div>'
            }
            if (index === 2) {
                data = '<div class="row"><div class="col-md-4 cell" contenteditable="true" @click="addInupts" id="'+id1+'"><p>kandarp</p></div><div class="col-md-4 cell" contenteditable="true" @click="addInupts" id="'+id3+'"><p>kandarp</p></div><div class="col-md-4 cell" contenteditable="true" @click="addInupts" id="'+id3+'"><p>kandarp</p></div></div>';
            }
